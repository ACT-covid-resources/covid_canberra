...
allfiles <- list.files(here::here("data/October/"), pattern=c("table_", ".csv"), full.names = TRUE)
...
# Save csv
write.csv(res, paste0('./data/', Sys.Date(),'lastupdate_resOctober.csv'))
res <- read.csv(paste0('./data/', Sys.Date(),'lastupdate_resOctober.csv'))
